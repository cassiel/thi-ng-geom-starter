...
(/Tangle/ does a save; since we can save while in sub-edit, perhaps a better way would be an ~after-save-hook~ to tangle on save, avoiding an infinite recursive loop if possible...)
...
See the [[file:src/cljs/thi_ng_geom_starter/core.org][main application]]. (That's a relative file link, not a URL.) This is the entry point for the Leiningen build. We (currently) have no Clojure source code at all - everything is ClojureScript.
...
- Evaluated source files won't carry line numbers that relate closely to code blocks, potentially making debugging a little tricky. (That's already true of REPL evaluation via ~C-M-x~.) I actually keep a second independent editor open on a second screen which auto-refreshes the exported code files, so that I can quickly eyeball what ClojureScript is seeing. (I've found [[http://brackets.io/][Brackets]] to work well.)

- I'm a little in the dark on Clojure reloading - I generally use ~cider-load-file~ (~C-c C-l~ in Clojure mode). Again, perhaps that needs to be tied into /tangle/ exporting somehow. ([[https://github.com/pyronicide/lein-autoreload][lein-autoreload]] might help.)
